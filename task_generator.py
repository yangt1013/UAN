...
def mini_imagenet_folders():
    train_folder = '/home/tianyang/桌面/目标识别mini格式/小样本目标识别2/LearningToCompare_FSL-master 1/datas/miniImagenet/train'
    test_folder = '/home/tianyang/桌面/目标识别mini格式/小样本目标识别2/LearningToCompare_FSL-master 1/datas/miniImagenet/val'
...
class MiniImagenetTask(object):
    ...
    def __init__(self, character_folders, num_classes, train_num,test_num):
        ...
        for c in class_folders:
            ...
            self.test_roots += samples[c][train_num:train_num+test_num]                  ######训练和测试放一起测试了？用到训练集内容了？
...
class FewShotDataset(Dataset):
    ...
    def __init__(self, task, split='train', transform=None, target_transform=None):
        ...
        self.task = task
        self.split = split
        self.image_roots = self.task.train_roots if self.split == 'train' else self.task.test_roots      ####图像输入?
...
def get_mini_imagenet_data_loader(task, num_per_class=1, split='train',shuffle = False):
    ...
    if split == 'train':
        ...
        sampler = ClassBalancedSampler(num_per_class, task.num_classes, task.test_num,shuffle=shuffle)    ###图像加载过程
