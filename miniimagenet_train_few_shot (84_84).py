...
parser.add_argument("-l","--learning_rate", type = float, default = 0.001)###0.04###0.001
...
class CNNEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc3 = nn.Linear(64 * 19* 19, 600)
        self.fc4 = nn.Linear(600, 59)
    ...
    def forward(self, x):
        ...
        # print(out3.shape)
        out3 = self.fc3(out3)
        out3 = self.fc4(out3)#-----------------------> cat[2]
        out2 = out2.view(out.size(0), -1)#------------------------cat[1]
...
class RelationNetwork(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(64*2, 64, kernel_size=3, padding=0),#-----,64,12,12
            nn.BatchNorm2d(64, momentum=1, affine=True),
            nn.ReLU(),
            nn.MaxPool2d(2))#64,6,6
        self.layer2 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=0),#------,64,4,4
            nn.BatchNorm2d(64, momentum=1, affine=True),
            nn.ReLU(),
            nn.MaxPool2d(2))#-------64,2,2
        self.fc1 = nn.Linear(23739, hidden_size)
    ...
    def forward(self, x1,x2):
        ...
        # print(x2.shape)
        out = self.layer1(x1)
        out = self.layer2(out)  ####此处原来cat融合后第二个conv block的输出-------64,2,2
        out = out.view(out.size(0), -1)#---------64*4
        # print(out.shape)
        # print('111',x2.shape)
        out_cat = torch.cat([x2,out],1)
        # print(out_cat.shape)

        out = F.relu(self.fc1(out_cat))
...
def main():
    ...
    metatrain_folders,metatest_folders = tg.mini_imagenet_folders()     #####此处用test文件？？？？
    ...
    feature_encoder_scheduler = StepLR(feature_encoder_optim,step_size=100000,gamma=0.5)###0.5
    relation_network_optim = torch.optim.Adam(relation_network.parameters(),lr=LEARNING_RATE)
    relation_network_scheduler = StepLR(relation_network_optim,step_size=100000,gamma=0.5)####0.5
    ...
    for episode in range(EPISODE):
        ...
        #####################
        # out,out3 = feature_encoder(Variable(samples).cuda(GPU))  # 25*64*19*19


        # calculate features
        sample_features,cat1 = feature_encoder(Variable(samples).cuda(GPU)) # 25*64*19*19
        # print(sample_features.shape)
        # print(cat1.shape)
        sample_features = sample_features.view(CLASS_NUM,SAMPLE_NUM_PER_CLASS,FEATURE_DIM,19,19)
        # print('cat1', cat1[1].shape)
        sample_features = torch.sum(sample_features,1).squeeze(1)
        # print(sample_features.shape)
        batch_features ,_= feature_encoder(Variable(batches).cuda(GPU)) # 20x64*5*5
        #################################################################
        # cat2 = cat2.unsqueeze(0).repeat(CLASS_NUM, 1, 1)
        # cat2 = torch.transpose(cat2, 0, 1)
        ...
        ####################################################################
        cat1 = cat1.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, 23163)
        cat1 = torch.sum(cat1, 1).squeeze(1)
        cat1 = cat1.unsqueeze(0).repeat(BATCH_NUM_PER_CLASS * CLASS_NUM, 1, 1)
        # print('cat2', cat2.shape)
        # print('cat1', cat1.shape)
        cat1 = cat1.view(250, 23163)
        # cat1 = torch.cat((cat1, cat2), 2).view(-1, 23163)
        # print('cat1', cat1.shape)
        #################################################################################
        relations = relation_network(relation_pairs,cat1).view(-1,CLASS_NUM)
        ...
        if (episode)%5000 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                for test_images,test_labels in test_dataloader:
                    ...
                    # calculate features
                    sample_features ,cat1= feature_encoder(Variable(sample_images).cuda(GPU)) # 5x64
                    sample_features = sample_features.view(CLASS_NUM,SAMPLE_NUM_PER_CLASS,FEATURE_DIM,19,19)
                    sample_features = torch.sum(sample_features,1).squeeze(1)
                    test_features,_ = feature_encoder(Variable(test_images).cuda(GPU)) # 20x64
                    ...
                    cat1 = cat1.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, 23163)
                    cat1 = torch.sum(cat1, 1).squeeze(1)
                    cat1 = cat1.unsqueeze(0).repeat(batch_size, 1, 1)
                    # print('cat2', cat2.shape)
                    # print('cat1', cat1.shape)
                    cat1 = cat1.view(125, 23163)
                    relation_pairs = torch.cat((sample_features_ext,test_features_ext),2).view(-1,FEATURE_DIM*2,19,19)
                    relations = relation_network(relation_pairs,cat1).view(-1,CLASS_NUM)

                    _,predict_labels = torch.max(relations.data,1)#-------->tensor
